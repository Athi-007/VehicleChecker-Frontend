...
export default function ReportPage() {
  ...
  // Snapshot data may be passed via navigation state (from Index page).
  // If not (e.g. direct URL), we re-fetch it.
  const [snapshotData, setSnapshotData] = useState<SnapshotBaseResponse | null>(
    (location.state as any)?.snapshotData ?? null
  );
  ...
  useEffect(() => {
    ...
    if (snapshotData) return; // already have data from nav state
    ...
    vehicleService
      ...
      .catch(() => {
        setError("Network error – could not load vehicle data.");
      })
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-10 max-w-5xl">
        ...
        {snapshotData && !loading && (
          <VehicleReport registration={reg} snapshotData={snapshotData} />
        )}
      </main>
